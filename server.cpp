...
static void do_something(int connfd) {
    char rbuf[64] = {};
    ssize_t n = read(connfd, rbuf, sizeof(rbuf) - 1); // can replace read with recv
    if (n < 0) {
        msg("read() error");
        return;
    }
    ...
    char wbuf[] = "world";
    write(connfd, wbuf, strlen(wbuf)); // can replace write with send
}
...
int main() {
    ...
    // listen: socket actually created here
